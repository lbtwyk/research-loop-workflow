...
The owner's Desktop App uses a separate `/home/tianhup/.codex-lbtwyk`
`CODEX_HOME`, including its own config, model instructions, mutable task state,
daemon, and control socket. Start or inspect it through `codex-lbtwyk`; connect
the App through `codex-lbtwyk app-server proxy`. The owner defaults are
`gpt-6-astra` with medium reasoning. Keep the ordinary `codex` executable and
