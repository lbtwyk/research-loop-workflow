...
- DEFAULT_DERIVATION_DOC = `docs/research/DERIVATION_PACKAGE.md` when the
  repository contains `docs/research/WORKFLOW.md`; otherwise
  `DERIVATION_PACKAGE.md` in the project root
...
3. if an explicit `EXP-ID` is in scope, run
   `scripts/experiment_ledger.py outline <EXP-ID>` and use its intended claim
   and relevant theory references without changing lifecycle or outcome
4. `docs/research/DERIVATION_PACKAGE.md` in a repository with the canonical
   research workflow; otherwise `DERIVATION_PACKAGE.md` in project root
...
If the user does not specify a target, use the default above. Do not create a
parallel experiment tracker or treat a coherent derivation as scientific
acceptance.
